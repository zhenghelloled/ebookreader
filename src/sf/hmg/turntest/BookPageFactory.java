...
public class BookPageFactory 
{
	...
	public BookPageFactory(int m_textColor,int m_fontSize) 
	{
		mWidth = 320;
		mHeight = 480;
...
//		nextPage();
	...
	protected void pageUp()
	{
		...
		while (lines.size() > mLineCount)
		{
			try
			{
				m_mbBufBegin += lines.get(0).getBytes(m_strCharsetName).length;lines.remove(0);
			}
			catch (UnsupportedEncodingException e)
			{
				e.printStackTrace();
